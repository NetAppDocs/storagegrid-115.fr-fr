...
Package de restauration `.zip` fichier
 a| 
Vous devez télécharger le dernier progiciel de récupération `.zip` fichier (`sgws-recovery-package-_id-revision_.zip`). Vous pouvez utiliser le fichier du progiciel de récupération pour restaurer le système en cas de défaillance.
