...
summary: 'Lorsqu"un client S3 émet une requête DE restauration POST-objet, une requête est envoyée afin de restaurer un objet à partir du stockage AWS Glacier vers un pool de stockage cloud. Ce message est émis par le serveur si la transaction a réussi.' 
---
= SPO : BORNE S3
:allow-uri-read: 
:icons: font
...
[role="lead"]
Lorsqu'un client S3 émet une requête DE restauration POST-objet, une requête est envoyée afin de restaurer un objet à partir du stockage AWS Glacier vers un pool de stockage cloud. Ce message est émis par le serveur si la transaction a réussi.
