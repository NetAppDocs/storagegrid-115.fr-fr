...
---
= Spécification des mots de passe système StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Vous devez disposer du mot de passe de provisionnement pour les procédures d'installation, d'extension et de maintenance, y compris pour le téléchargement du package de reprise. Il est donc important de stocker la phrase secrète de provisionnement dans un emplacement sécurisé.
...
* La console de ligne de commande générée de manière aléatoire et les mots de passe SSH sont stockés dans le `Passwords.txt` fichier dans le package de restauration.
...
IMPORTANT: Vous êtes invité à télécharger le fichier du progiciel de récupération (`sgws-recovery-package-id-revision.zip`) Après avoir cliqué sur *installer* sur la page Résumé. Vous devez télécharger ce fichier pour terminer l'installation. Les mots de passe requis pour accéder au système sont stockés dans le `Passwords.txt` Fichier, contenu dans le fichier du progiciel de récupération.
